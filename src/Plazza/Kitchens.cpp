...
std::string operator<<(std::string &str, Pizza &pizza)
{
    str = pizza.serialize();
    return (str);
}

Kitchens::Kitchens(std::size_t nbCooks, std::size_t kitchenID, std::size_t cookingTimeMultiplier, std::size_t timeToRestock) : _cooks(nbCooks)
{
...
void Kitchens::checkDestroyKitchen()
{
    if (!isKitchenWorking()) {
...
bool Kitchens::isKitchenWorking()
{
...
std::string preparePizza(Pizza pizza, std::size_t doe, std::size_t timeToRestock)
{
    auto t2 = Clock::now();
    if (doe < 0)
...
void Kitchens::sendOrderToCooks(std::vector<std::shared_ptr<Pizza>> kitchenOrder)
{
...
void Kitchens::sendReceipt(std::vector<std::string> &cookedOrders)
{
...
std::vector<std::string> Kitchens::getCookedOrders()
{
    std::vector<std::string> cookedOrders;
    std::string tmp;
...
std::size_t Kitchens::findAssignedKitchen(std::string &order)
{
    std::size_t pos = 0;
...
bool Kitchens::isOrderAssignedHere(std::string &order)
{
...
std::vector<std::shared_ptr<Pizza>> Kitchens::getOrderFromReception(std::vector<std::string> &allOrders)
{
    Utils utils;
    std::vector<std::shared_ptr<Pizza>> kitchenOrders;
